...
function getMapboxCenter(layout: unknown): { lat: number; lon: number } | null {
...
function getMapboxZoom(layout: unknown): number | null {
	const r = asRecord(layout);
	const mapbox = r ? asRecord(r.mapbox) : null;
...
function getLayoutMeta(layout: unknown): unknown {
	const r = asRecord(layout);
	return r?.meta;
}

function getRelayoutCenter(
...
function getRelayoutZoom(event: Record<string, unknown>): number | null {
	const z = event["mapbox.zoom"];
	return typeof z === "number" ? z : null;
}

function calcBboxFromCenterZoom(
...
function RouteComponent() {
	...
	const [telemetrySummary, setTelemetrySummary] = useState<
		TelemetrySummaryRow[] | null
	>(null);
	const [telemetrySlowest, setTelemetrySlowest] = useState<
		TelemetrySlowestRow[] | null
	>(null);
	...
	const loadTelemetry = useCallback(async () => {
		try {
			...
			setTelemetrySummary(
				Array.isArray(sRows) ? (sRows as TelemetrySummaryRow[]) : [],
			);
			setTelemetrySlowest(
	...
	const isInvokePendingRef = useRef(false);
	...
	const schedulePlotRefresh = useCallback(
		(next: {
			...
			if (isInvokePendingRef.current || partialMessage !== null) return;
		...
		[currentHighlight, engine, getViewportSize, partialMessage, scenarioId],
	);
	...
	useEffect(() => {
		isInvokePendingRef.current = isPending;
	}, [isPending]);
	...
	}, [defaultExamplePrompts, scenarioId, getViewportSize, schedulePlotRefresh]);
				...
				{telemetryOpen && (
					<div className="absolute top-2 right-2 z-10 rounded-md border border-border bg-background/95 px-3 py-2 text-xs w-[420px] max-w-[90%] max-h-[85%] overflow-auto">
						...
						<div className="text-muted-foreground mb-2">
							If the backend is running, prefer this panel/API. DuckDB telemetry
							DB is locked for external readers while the backend writes to it.
						</div>
						...
						<div className="space-y-1 mb-3">
							{(telemetrySummary ?? []).map((row) => (
								...
								>
									...
									<div className="text-muted-foreground">
										cache hit: {row.cacheHitRate?.toFixed?.(2) ?? "?"}, avg
										payload: {row.avgPayloadKB?.toFixed?.(1) ?? "?"}KB
									</div>
								</div>
							))}
						...
						<div className="space-y-0.5 text-muted-foreground">
							...
							{currentStats()?.payloadBytes && (
								<div>
									...
									<span className="text-foreground">
										{Math.round(
											(currentStats()?.payloadBytes as number) / 1024,
										)}
										KB
									</span>
								</div>
