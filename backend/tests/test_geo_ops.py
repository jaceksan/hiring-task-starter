...
def test_point_in_polygon_returns_bool():
    ...
    u = index.polygon_union_for_aoi("polys", BBox(min_lon=-1, min_lat=-1, max_lon=3, max_lat=3))
...
def test_distance_to_nearest_point_is_non_negative_meters():
    ...
    bundle = LayerBundle(
        layers=[
            Layer(id="targets", kind="points", title="Targets", features=[near], style={}),
            Layer(id="sources", kind="points", title="Sources", features=[pt], style={}),
        ]
    )
    index = build_geo_index(bundle)
